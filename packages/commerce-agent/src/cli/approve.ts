...
function main(): void {
  ...
  if (!proposalId) throw new Error('Usage: --proposal <proposal_id> --confirm-hash <content_hash> [--reason <text>]')
  ...
  print(runtime.approvals.approve({ proposalId, principal: runtime.approverPrincipal, reason, confirmHash: confirmHash! }))
}
